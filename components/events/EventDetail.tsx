...
export default function EventDetail({ event, open, isAdmin, onClose, onStatusChange, onRSVPChange }: EventDetailProps) {
  ...
  async function review(status: 'APPROVED' | 'REJECTED') {
    ...
    try {
      ...
      setReviewError('Could not update this proposal yet.')
    } finally {
      setReviewing(false)
    }
  }
  ...
  return (
    <div className="fixed inset-0 z-70 flex items-end justify-center bg-black/68 px-0 backdrop-blur-md sm:items-center sm:px-6" role="dialog" aria-modal="true" aria-label="Event detail">
      <button type="button" className="absolute inset-0 cursor-default" onClick={onClose} aria-label="Close event detail" />
      <article
      ...
      >
        ...
        <div className="pr-10">
          <p className="text-eyebrow mb-2">{event.category.toLowerCase()}</p>
        ...
        <div className="mt-6 flex items-center gap-3 rounded-xl p-3" style={{ background: 'rgba(255,255,255,0.035)', border: '1px solid rgba(255,255,255,0.07)' }}>
          ...
          <div>
            <p className="text-xs" style={{ color: 'var(--ghost)' }}>Proposed by</p>
        ...
        <div className="mt-6">
          <div className="mb-3 flex items-center justify-between gap-3">
            <p className="text-data-label">Attendees</p>
          ...
          <div className="flex items-center gap-3">
            <div className="flex min-h-10 items-center pl-2">
              ...
              {visibleAttendees.length === 0 && <span className="text-sm" style={{ color: 'var(--ghost)' }}>No RSVPs yet.</span>}
            </div>
            {hiddenCount > 0 && <span className="text-xs" style={{ color: 'var(--ghost)' }}>and {hiddenCount} others</span>}
          </div>
        </div>
        ...
        <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          {event.spotsRemaining !== null && (
            <span className="text-xs" style={{ color: 'var(--ghost)' }}>{event.spotsRemaining} spots remaining</span>
          )}
        </div>

        {isAdmin && event.status === 'PENDING' && (
          <div className="mt-6 rounded-xl p-4" style={{ background: 'rgba(255,255,255,0.035)', border: '1px solid rgba(255,255,255,0.08)' }}>
            <p className="text-data-label mb-3">Admin review</p>
            {rejecting && (
              <textarea
